<?php include('session.php');
	...
	error_reporting(0);
    if($_GET['edit']=="yes"){
        $sqll="select * from slider_picture where fld_id='".$_GET['id']."' limit 1";
	...
	if(isset($_REQUEST['delete'])) {   
	    $id=$_GET['delete'];
	    $del_query=mysql_query("select * from slider_picture where fld_id='$id'");
	    $del_row = mysql_fetch_array($del_query);
	    mysql_query("delete from slider_picture where fld_id='$id'");
	    if(file_exists("../slider_picture//$del_row[fld_img]")){
	    unlink("../slider_picture//$del_row[fld_img]");
	    header("location: slider_picture.php?confirm=Image Deleted");
	    }
	}
?>
<?php
    if(isset($_REQUEST['submitt']) && isset($_POST['Upload'])){
        ...
        $insert=mysql_query("insert into slider_picture (fld_title,fld_desc,fld_img) values ('$texttitle','$textdetails','$file')") or die(mysql_error());
        if($insert){
        	header("location:slider_picture.php?msg=Image Uploaded Successfully!");
        }
    }
?>
<?php
	...
	$sql = mysql_query("update slider_picture set fld_title = '$texttitle', fld_desc = '$textdetails', fld_img = '$file' where fld_id = '$id'")or die(mysql_error());
?>
<script type="text/javascript">
	window.location ="slider_picture.php?id=<?php echo $id;?>&edit=<?php echo $yes;?>&confirm=Image Updated!";
</script>
<?php } ?>
<!DOCTYPE html>
							...
							<div class="portlet box yellow">
								...
								<div class="portlet-body form">	
	                                   <form role="form"  method="post" enctype="multipart/form-data" class="form-horizontal">
											...
											<div class="control-group">
												<div class="controls" style="height:100px; width:100px; padding:2px;">
													<img src="../slider_picture/<?php echo $file; ?>" width="100px" height="80px"  border="1px solid #4B8DF8" alt="No Image Available">
												</div>
											</div><?php } ?>	
	                                    	<div class="control-group">
												...
												<div class="controls">
													...
													<p style="color:red;">Width: <b>208px</b> height: <b>200px</b>;</p>
												</div>
											</div>
											<div class="form-actions">
											 	 
												 <?php if(empty($_GET['edit']) == "yes"){ ?>  
													<button type="submit"  name="Upload" class="btn yellow">Upload</button>
													<input type="hidden" name="submitt" value="1">
													<button type="reset" value="" class="btn">Cancel</button>

												<?php } if($_GET['edit'] == "yes"){ ?>	
													<button type="submit"  name="submit" class="btn yellow">Upload</button>
										...
										<thead class="flip-content">
											...
											<tbody>
						                       <?php
					                            	...
					                            	$listresult=mysql_query("select * from slider_picture order by fld_id desc");
												...
												<tr>
													...
													<td>
													<?php if($row['fld_img']){ ?>
														<img src="../slider_picture/<?php echo "$row[fld_img]";?>" width="35" height="35" >
													<?php } else { ?>
														 <img src="../slider_picture/no_image.jpg" width="35" height="35" >
													<?php } ?>	 
													</td>
													<td><a href ='slider_picture.php?id=<?php echo "$row[fld_id]";?>&edit=yes' title='Edit'>Edit</a></td>
