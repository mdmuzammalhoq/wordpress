...
<?php include 'include/objective_inner.php'; ?>
						...
						<h2 class="art-PostHeader"><a href="#" rel="bookmark" title="District Function">Projects</a></h2>
						...
						<?php 
							// $sql = mysql_query("select * from projects order by fld_id desc");
							// while($row = mysql_fetch_array($sql)){ 
							$query = "select * from projects order by fld_id desc";
							$photo = $db->select($query);
							if ($photo) {
								while ($result = $photo->fetch_assoc()) {
						?>	
						<div class="ngg-gallery-thumbnail" >
						<a href="img_201415/project/<?php echo $result['fld_img']; ?>" title="<?php echo $result['fld_title']; ?>" class="thickbox" rel="set_9" > <img title="1" alt="1" src="img_201516/<?php echo $result['fld_img']; ?>" width="120" height="90" /> </a>
						</div>
						<?php } } ?>
						</div>
						</div>
